...
func ctorFreedarwin(kernelSrc, kernelObj string, ignores []*regexp.Regexp) (Reporter, []string, error) {
	ctx := &darwin{
		kernelSrc: kernelSrc,
		kernelObj: kernelObj,
		ignores:   ignores,
...
func (ctx *darwin) Parse(output []byte) *Report {
	...
	title, corrupted, _ := extractDescription(output[rep.StartPos:], oops, darwinStackParams)
	rep.Title = title
...
var darwinStackParams = &stackParams{}
